...
`AuthProvider` also exposes `deleteAccount` (used by a "Delete account" button next to Log out in `AppShell`): deletes all of the user's Firestore expense docs first (batched, while still authenticated — Firestore's rules require `request.auth.uid == userId`), then calls `deleteUser` on the Firebase Auth account. Gated behind a native `confirm()` since it's irreversible. Note this doesn't revoke Google's own OAuth consent grant for the app (that's tracked independently by Google, not Firebase) — signing in again with the same Google account afterward won't re-show the permissions screen, since Google still remembers the prior authorization.
